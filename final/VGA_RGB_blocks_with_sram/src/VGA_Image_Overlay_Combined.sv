...
module VGA_Image_Overlay_Combined #(
    ...
    // End game images parameters
    parameter SURVIVED_ADDR = 21'd1382401,      // SRAM address for survived image (to be set)
    parameter DIED_ADDR = 21'd1610881,          // SRAM address for died image (to be set)
    parameter RETRY_ADDR = 21'd1811200,         // SRAM address for retry image (to be set)
    parameter SURVIVED_WIDTH = 640,
    parameter SURVIVED_HEIGHT = 357,
    parameter DIED_WIDTH = 640,
    parameter DIED_HEIGHT = 313,
    parameter RETRY_WIDTH = 280,
    parameter RETRY_HEIGHT = 153
)(
    input logic i_clk,
    input logic i_rst_n,
    ...
    // End game signals
    input logic i_win_signal,
    input logic i_lose_signal,
    
    // Hit flash effect
    input logic i_zombie_hit,
    ...
    // Background SRAM (Stage 1 -> 2) - shared with end game images
	...
	output logic o_died_started  // High for one cycle when a zombie starts dying (becomes gray)
);
    ...
    localparam DIED_POS_X = 320;      // (1280 - 640) / 2
    localparam DIED_POS_Y = 80;       // Top-middle
    localparam RETRY_POS_X = 500;     
    localparam RETRY_POS_Y = 400;     // Bottom-middle

    // ============================================================
    //  Hit Flash Effect: Red tint when zombie bites player
    // ============================================================
    // 0.2 seconds at 74.25 MHz = 14,850,000 cycles (need 24 bits: 2^24 = 16,777,216)
    localparam HIT_FLASH_DURATION = 24'd14850000;  // 0.2 seconds at 74.25 MHz
    
    logic hit_flashing;  // Flash state: 1 = currently flashing red
    logic [23:0] hit_flash_timer;  // Timer for flash duration
    logic hit_prev;  // Previous hit signal for edge detection
    ...
    always_ff @(posedge i_clk) begin
        if (~i_rst_n) begin
            ...
            // Update flash timer
            if (hit_flashing) begin
                if (hit_flash_timer < HIT_FLASH_DURATION) begin
                    hit_flash_timer <= hit_flash_timer + 24'd1;
                end else begin
                    // Timer expired: stop flashing
                    hit_flashing <= 1'b0;
                    hit_flash_timer <= 24'd0;
                end
            end
    ...
    logic [20:0] bg_pixel_offset_from_base_s0;  // Pixel offset from base address
    ...
    // --- 0.2.5 End Game Images Calc ---
    logic in_survived_s0, in_died_s0, in_retry_s0;
    logic [10:0] survived_lx_s0, died_lx_s0, retry_lx_s0;
    logic [9:0] survived_ly_s0, died_ly_s0, retry_ly_s0;
    logic [20:0] end_img_addr_s0;
    logic end_img_active_s0;
    ...
    assign end_img_active_s0 = in_retry_s0 || in_survived_s0 || in_died_s0;
    always_comb begin
        if (in_retry_s0) begin
            end_img_addr_s0 = RETRY_ADDR + (retry_ly_s0 * RETRY_WIDTH) + retry_lx_s0;
            end_img_type_s0 = 2'd3;  // Retry
        end else if (in_survived_s0) begin
            end_img_addr_s0 = SURVIVED_ADDR + (survived_ly_s0 * SURVIVED_WIDTH) + survived_lx_s0;
            end_img_type_s0 = 2'd1;  // Survived
        end else if (in_died_s0) begin
            end_img_addr_s0 = DIED_ADDR + (died_ly_s0 * DIED_WIDTH) + died_lx_s0;
            end_img_type_s0 = 2'd2;  // Died
        end else begin
            end_img_addr_s0 = 21'd0;
            end_img_type_s0 = 2'd0;  // None
        end
    end
    ...
    logic [1:0]  bg_pixel_offset_s1;  // Pixel offset for 4-bit mode
    ...
    // End game signals
    logic [20:0] end_img_addr_s1;
    logic        end_img_active_s1;
    logic [1:0]  end_img_type_s1;

    always_ff @(posedge i_clk) begin
        if (~i_rst_n) begin
            bg_addr_s1 <= 0; bg_pixel_offset_s1 <= 0; active_video_s1 <= 0; in_aim_s1 <= 0; at_kill_target_s1 <= 0;
            end_img_addr_s1 <= 0; end_img_active_s1 <= 0; end_img_type_s1 <= 0;
        end else begin
            // Zombie Data
            for(int k=0; k<4; k++) begin
            ...
            end_img_type_s1 <= end_img_type_s0;
        end
    end
    ...
    // Multiplex between background and end game addresses
    // Priority: end game images > background
    logic [20:0] final_sram_addr_s1;
    
    // Select address source:
    // - If in end game block: use end_img_addr_s1 (8-bit per pixel)
    // - Else if start screen: use bg_addr_s1 (4-bit per pixel)
    // - Else: use bg_addr_s1 (8-bit per pixel for game)
    assign final_sram_addr_s1 = end_img_active_s1 ? end_img_addr_s1 : bg_addr_s1;
    
    assign o_bg_sram_ce_n = 1'b0;
    assign o_bg_sram_oe_n = 1'b0;
    assign o_bg_sram_we_n = 1'b1;
    assign o_bg_sram_addr = final_sram_addr_s1[20:1];  // Word address
    
    // Byte select logic:
    // - End game images: always 8-bit mode (select byte)
    // - Start screen (not started): 4-bit mode (read both bytes)
    // - Game (started): 8-bit mode (select byte)
    // CRITICAL: Use started_s1 that matches bg_addr_s1 to ensure synchronization
    // bg_addr_s1 was calculated in Layer 0 using i_started, then pipelined to Layer 1
    // So started_s1 should match the mode used to calculate bg_addr_s1
    assign o_bg_sram_lb_n = (end_img_active_s1 || started_s1) ? final_sram_addr_s1[0] : 1'b0;
    assign o_bg_sram_ub_n = (end_img_active_s1 || started_s1) ? ~final_sram_addr_s1[0] : 1'b0;
    ...
    logic [1:0]  bg_pixel_offset_s2;  // Pixel offset for 4-bit mode
    logic        in_aim_s2;
    logic        active_video_s2;
    logic        at_kill_target_s2;
    logic        started_s2;  // Pipeline i_started signal
    // End game signals
    logic [20:0] end_img_addr_s2;
    logic        end_img_active_s2;
    logic [1:0]  end_img_type_s2;

    always_ff @(posedge i_clk) begin
        if (~i_rst_n) begin
            active_video_s2 <= 0; at_kill_target_s2 <= 0; bg_pixel_offset_s2 <= 0;
            end_img_addr_s2 <= 0; end_img_active_s2 <= 0; end_img_type_s2 <= 0;
        end else begin
            ...
            bg_pixel_offset_s2 <= bg_pixel_offset_s1;
            // Pass Aim & Control
            in_aim_s2 <= in_aim_s1;
            ...
            // End game
            end_img_addr_s2 <= end_img_addr_s1;
            end_img_active_s2 <= end_img_active_s1;
            end_img_type_s2 <= end_img_type_s1;
        end
    end
    ...
    logic died_started_pulse;  // Pulse signal when a zombie starts dying
        ...
        if (~i_rst_n) begin
            ...
            if (kill_detected_s2) begin
                ...
                if ((killed_idx < MAX_ZOMBIES) && !zombie_flashing[killed_idx]) begin
                    ...
                    died_started_pulse <= 1'b1;  // Pulse when zombie starts dying
                end
            end
            ...
            // Clear pulses after one cycle
            if (kill_en_reg) begin
                kill_en_reg <= 1'b0;
            end
            if (died_started_pulse) begin
                died_started_pulse <= 1'b0;
            end
        end
    end
    ...
    // ------------------------------------------------------------
    logic [15:0] sram_data_latched_s3;  // SRAM data (could be BG or end game)
    logic        bg_low_byte_s3;        // For 8-bit mode: select byte (BG or end game)
    logic [1:0]  bg_pixel_offset_s3;    // For 4-bit mode: select nibble (0-3) (start screen only)
    ...
    // End game signals
    logic        end_img_active_s3;     // Flag: is this pixel from end game image?
    logic [20:0] end_img_addr_s3;       // End game address (for byte select)
    logic [1:0]  end_img_type_s3;       // Image type: 0=none, 1=survived, 2=died, 3=retry
    ...
    always_ff @(posedge i_clk) begin
        if (~i_rst_n) begin
            ...
            // 2. Shared SRAM Data Latch (could be BG or end game data)
            // NOTE: SRAM read latency - data is available 1 cycle after address is output
            // Address is output in Layer 1, so data should be valid here in Layer 2->3 transition
            // However, if SRAM has 2-cycle latency, we may need to add an extra stage
            sram_data_latched_s3 <= io_bg_sram_dq; 
            bg_low_byte_s3       <= bg_addr_s2[0]; // Needed to split 16-bit to 8-bit (game mode or end game)
            bg_pixel_offset_s3   <= bg_pixel_offset_s2; // Pixel offset for 4-bit mode (start screen)
            ...
            // 4. End Game Tracking
            end_img_active_s3 <= end_img_active_s2;
            end_img_addr_s3 <= end_img_addr_s2;
            end_img_type_s3 <= end_img_type_s2;
    ...
    logic [3:0] bg_pixel_4bit_s3;  // Temporary 4-bit pixel value for start screen
    
    // For start screen (4-bit mode): Extract 4-bit nibble from 16-bit word based on pixel offset
    // For game (8-bit mode): Extract 8-bit byte based on low byte selector
    always_comb begin
        // Initialize to avoid latches
        bg_pixel_4bit_s3 = 4'b0000;
        bg_pixel_index_s3 = 8'h00;
        
        if (started_s3) begin
            // Game mode: 8-bit per pixel, 2 pixels per 16-bit word
            bg_pixel_index_s3 = (bg_low_byte_s3 == 1'b0) ? sram_data_latched_s3[7:0] : sram_data_latched_s3[15:8];
        end else begin
            // Start screen mode: 4-bit per pixel, 4 pixels per 16-bit word
            // Extract 4-bit nibble based on pixel offset (0-3)
            case (bg_pixel_offset_s3)
                2'b00: bg_pixel_4bit_s3 = sram_data_latched_s3[3:0];    // Pixel 0 (bits 3:0)
                2'b01: bg_pixel_4bit_s3 = sram_data_latched_s3[7:4];    // Pixel 1 (bits 7:4)
                2'b10: bg_pixel_4bit_s3 = sram_data_latched_s3[11:8];   // Pixel 2 (bits 11:8)
                2'b11: bg_pixel_4bit_s3 = sram_data_latched_s3[15:12];  // Pixel 3 (bits 15:12)
            endcase
            // Extend 4-bit to 8-bit for palette lookup (pad with zeros)
            bg_pixel_index_s3 = {4'b0000, bg_pixel_4bit_s3};
        end
    end

    // --- 3.2 End Game Pixel Index Parse ---
    // End game images use 8-bit per pixel (same as game mode)
    // When end_img_active_s3 is true, sram_data_latched_s3 contains end game image data
    logic [7:0] end_img_pixel_index_s3;
    assign end_img_pixel_index_s3 = (end_img_addr_s3[0] == 1'b0) ? sram_data_latched_s3[7:0] : sram_data_latched_s3[15:8];

    // --- 3.3 Zombie BRAM Data ---
	...
	// End game signals
	logic [7:0] end_img_pixel_index_s4;
	logic       end_img_active_s4;
	logic [1:0] end_img_type_s4;
    ...
    always_ff @(posedge i_clk) begin
        if(~i_rst_n) begin 
			...
			// End game
			end_img_pixel_index_s4 <= end_img_pixel_index_s3;
			end_img_active_s4 <= end_img_active_s3;
			end_img_type_s4 <= end_img_type_s3;
        end
    end
    ...
    // --- 4.2 End Game Palettes Instantiation ---
    logic [15:0] survived_rgb565_s5;  // Returns in Layer 5
    logic [15:0] died_rgb565_s5;      // Returns in Layer 5
    logic [15:0] retry_rgb565_s5;     // Returns in Layer 5
    logic [15:0] end_rgb565_s5;       // Multiplexed output
    
    // Survived palette
    win_palette win_pal (
        .clock(i_clk), 
        .address(end_img_pixel_index_s4), 
        .q(survived_rgb565_s5)
    );
    
    // Died palette
    lose_palette lose_pal (
        .clock(i_clk), 
        .address(end_img_pixel_index_s4), 
        .q(died_rgb565_s5)
    );
    ...
    // Retry palette
    retry_palette retry_pal (
        .clock(i_clk), 
        .address(end_img_pixel_index_s4), 
        .q(retry_rgb565_s5)
    );
    
    // Multiplex based on image type (in Layer 5)
    assign end_rgb565_s5 = (end_img_type_s4 == 2'd1) ? survived_rgb565_s5 :
                           (end_img_type_s4 == 2'd2) ? died_rgb565_s5 :
                           (end_img_type_s4 == 2'd3) ? retry_rgb565_s5 :
                           16'd0;
    ...
    // End game signals
    logic [15:0] end_img_rgb565_s5;
    logic        use_end_img_s5;
    ...
    always_ff @(posedge i_clk) begin
        if(~i_rst_n) begin
            ...
            // End game
            end_img_rgb565_s5 <= end_rgb565_s5;  // Latch multiplexed palette output
            use_end_img_s5 <= end_img_active_s4 && (end_img_pixel_index_s4 != 8'h00);  // Check for transparency
        end
    end
    ...
    assign end_r = {end_img_rgb565_s5[15:11], end_img_rgb565_s5[15:13]};
    assign end_g = {end_img_rgb565_s5[10:5],  end_img_rgb565_s5[10:9]};
    assign end_b = {end_img_rgb565_s5[4:0],   end_img_rgb565_s5[4:2]};
    ...
    always_comb begin
        logic [7:0] base_r, base_g, base_b;  // Base color before hit flash
        
        // Priority: aim > end game > zombie > background
        if (use_aim) begin
            base_r = aim_r; base_g = aim_g; base_b = aim_b;
        end else if (use_end_img_s5) begin
            // End game images (survived/died/retry)
            base_r = end_r; base_g = end_g; base_b = end_b;
            ...
            if (zombie_is_flashing_s5) begin
                if (zombie_show_gray_s5) begin
                    ...
                    base_r = 8'd128; base_g = 8'd128; base_b = 8'd128;
                end else begin
                    // Show background color
                    base_r = bg_r; base_g = bg_g; base_b = bg_b;
                end
            end else begin
                // Normal zombie color
                base_r = z_r;   base_g = z_g;   base_b = z_b;
            end
        end else begin
            base_r = bg_r;  base_g = bg_g;  base_b = bg_b;
        end
        
        // Apply hit flash effect: add 50 to red channel (saturate at 255)
        if (hit_flashing) begin
            fin_r = (base_r > 8'd155) ? 8'd255 : (base_r + 8'd100);
            fin_g = base_g;
            fin_b = base_b;
        end else begin
            fin_r = base_r;
