module velocity_integrator (
    ...
    localparam CALIB_SAMPLES = 32;
    localparam CALIB_SHIFT = 5; // 2^5 = 32
                ...
                case (state)
                    ST_CALIB: begin
                        ...
                        if (sample_count == CALIB_SAMPLES - 1) begin
                            // Calculate average (division by bit shift)
                            ...
                            // Done calibration, reset velocity accumulators
                            v_acc_x <= 0;
                            v_acc_y <= 0;
                            v_acc_z <= 0;
                            calibrated <= 1;
                    ...
                    ST_RUN: begin
                        // Integrate: v_new = v_old + (acc - offset)
                        v_acc_x <= v_acc_x + (ax - offset_x);
                        v_acc_y <= v_acc_y + (ay - offset_y);
                        v_acc_z <= v_acc_z + (az - offset_z);
                        
                        // Output scaled down (lower 32 bits for now)
                        vx <= v_acc_x[31:0];
                        vy <= v_acc_y[31:0];
                        vz <= v_acc_z[31:0];
