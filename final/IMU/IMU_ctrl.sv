...
module mpu6050_ctrl #(
    ...
    //  簡單的取樣頻率分頻 (控制多久讀一次資料)
    // =========================================================
    // 假設想每約5ms讀一次資料 (視需求可以調整)
    localparam int SAMPLE_HZ = 200;                // 200次/秒 => 5ms
    ...
    // 用來控制「目前在做第幾次I2C動作」
    logic [3:0] byte_index;    // 0..13  共14 bytes
    logic [7:0] buf [0:13];    // 暫存加速度/溫度/陀螺資料
    ...
    always_comb begin
        ...
        case (state)
            ...
            ST_READ_BYTE: begin
                i2c_rw       = 1'b1;   // read
                i2c_reg_addr = REG_ACCEL_XOUT_H + byte_index;
                if (!i2c_busy) begin
                    i2c_start   = 1'b1;
                end
            end
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                ST_DONE_FRAME: begin
                    ...
                    acc_x  <= $signed({buf[0], buf[1]});
                    acc_y  <= $signed({buf[2], buf[3]});
                    acc_z  <= $signed({buf[4], buf[5]});
                    gyro_x <= $signed({buf[8], buf[9]});
                    gyro_y <= $signed({buf[10], buf[11]});
                    gyro_z <= $signed({buf[12], buf[13]});
